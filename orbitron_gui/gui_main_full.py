...
class OrbitronWindow(Ui_MainWindow, QtWidgets.QMainWindow):
    ...
    def __init__(self, orbitron_client: OrbitronTcpClient, *args, **kwargs):
        super(OrbitronWindow, self).__init__(*args, **kwargs)
        ...
        self.orbitron_stations: dict[StationName, StationInfo] = {}
        ...
        logger.info("Orbitron GUI is successfully initialized.")
    ...
    def gui_update_station_available_satellites(self, norad_ids: Iterable) -> None:
        ...
        self.orbitron_norad_ids_scroll_area.setWidget(self._satellites_widget)
    ...
    def clear_station_data_menu_button_slot(self) -> None:
        """Menu button slot to delete all data for selected ground station from Orbitron
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            try:
                self.orbitron_client.clear_ground_station_data(self.station_info.name)
            except TCPServerResponseError:
    ...
    def add_new_satellite_by_worker(
        ...
        """Add new satellite to Orbitron TCP server and on GUI by worker.
    ...
    def choose_station_by_dialog(self) -> Optional[StationInfo]:
        """Choose setuped ground station from Orbitron Server and setup new ground
        stations to Orbitron Server by Dialog.

        Returns:
            StationInfo: dataclass with parameters of selected ground station if station
                is chosen, else None
        """
        stations_dialog = ChooseStationDialog(self.orbitron_client)
    ...
    def get_station_available_satellites(
        ...
        """
        try:
            satellites = self.orbitron_client.get_station_satellites_info(station_name)
            satellites_info = {}
    ...
    def update_selected_station_available_satellites(self) -> None:
        """Get setuped satellites from Orbitron Server for selected ground station and
