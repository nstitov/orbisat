...
from ..exceptions.orbisat_exceptions import NewOrbisatDataError, NewOrbisatSetupError
from ..orbisat_services.communication import (
    CommParams,
    SatelliteStationComm,
    SessionParams,
)
from ..orbisat_services.ground_station import GroundStation
from ..orbisat_services.satellite import Satellite
...
class Orbisat:
    """A class used to represent OrbiSat

    Attributes:
        stations (dict[str, GroundStation]): Dict of setuped ground stations into
            OrbiSat with stations names as dict keys and instances of GroundStation as
            dict values
        satellites (dict[int, Satellite]): Dict of setuped satellites into OrbiSat with
        ...
        comms (dict[str, SatelliteStationComm]): Dict of setuped communications into
            OrbiSat between satellite (instance of Satellite) and ground station
    ...
    Methods:
        setup_ground_station(longitude, latitude, altitude[, min_elevation, name]): Add
            ground station (instance of GroundStation class) into OrbiSat. The default
            name of ground station is 'default'
        setup_satellite(norad_id[, uplink, downlink]): Add satellite (instance of
            Satellite class) into OrbiSat
        ...
        update_tles(norad_ids): Update TLE files for all NORAD ID in list of norad_ids.
            The satellites with required NORAD ID must setuped into OrbiSat
    ...
    Raises:
        NewOrbiSatSetupError: If OrbiSat hasn't setups for required satellite, ground
            station or communication
        NewOrbiSatDataError: If communication for required satellite and ground
    ...
    def _check_ground_station_setup(self, station_name: str) -> None:
        """Check ground station setup in OrbiSat."""
        try:
            if station_name not in self.stations:
                raise NewOrbisatSetupError(
                    f"Trying setup satellite for '{station_name}' ground station which "
                    f"hasn't setup in OrbiSat"
                )
        except NewOrbisatSetupError:
            logger.exception("OrbiSat hasn't required setups.")
            raise
        ...
        try:
            if norad_id not in self.satellites[station_name]:
                raise NewOrbisatSetupError(
                    ...
                    f"{norad_id} which hasn't setup in OrbiSat with '{station_name} "
                    f"ground station"
                )
        except NewOrbisatSetupError:
            logger.exception("OrbiSat hasn't required setups.")
    ...
    def _check_comm_setup_for_satellite_with_ground_station(
        ...
        try:
            if norad_id not in self.comms[station_name]:
                raise NewOrbisatSetupError(
                    f"OrbiSat hasn't communication for satellite with NORAD ID "
    ...
    def _check_comm_prediction_data(self, station_name: str, norad_id: int) -> None:
        ...
        try:
            if not self.comms[station_name][norad_id].comm_data:
                raise NewOrbisatDataError(
                    f"OrbiSat hasn't predicted data for communication between "
                    f"satellite with NORAD ID {norad_id} and '{station_name}' ground "
                    f"station."
                )
        except NewOrbisatDataError:
            logger.exception("OrbiSat data error.")
    ...
    def setup_ground_station(
        ...
        """Setup ground station to OrbiSat.
    ...
    def setup_satellite(
        ...
        """Setup satellite for ground station to OrbiSat by NORAD ID.

        Args:
            station_name (str): Name of ground station setuped into OrbiSat
        ...
        Raises:
            NewOrbiSatSetupError: If OrbiSat hasn't setup for required ground station
    ...
    def setup_comm(self, station_name: str, norad_id: int) -> None:
        ...
        Args:
            station_name (str): Name of ground station setuped into OrbiSat
            norad_id (int): NORAD ID setuped into OrbiSat satellite for ground station

        Raises:
            NewOrbiSatSetupError: If OrbiSat hasn't setups for required ground station
                or satellite
    ...
    def update_tles_by_spacetrack(
        ...
        for norad_id in norad_ids:
            if norad_id in self.satellites[station_name]:
                ...
                logger.warning(
                    f"OrbiSat hasn't setup for satellite with NORAD ID {norad_id} for "
    ...
    def predict_comm(
        ...
        Raises:
            NewOrbiSatSetupError: If OrbiSat hasn't communication setup for required
                satellite and ground station
...
def log_comm_data(
    ...
    """
    OrbiSat = OrbiSat()
    OrbiSat.setup_satellite(norad_id, uplink, downlink)
    OrbiSat.setup_ground_station(longitude, latitude, altitude, min_elevation, "test")
    OrbiSat.setup_comm(norad_id, "test")
    OrbiSat.predict_comm(norad_id, "test", start_dt, time_prediction, step_prediction)

    comm_data = OrbiSat.get_all_data(norad_id, "test")
...
if __name__ == "__main__":
    ...
    OrbiSat = OrbiSat()
    OrbiSat.setup_ground_station(
        longitude, latitude, altitude, min_elevation, station_name
    )
    OrbiSat.setup_satellite(station_name, norad_id, uplink, downlink)
    OrbiSat.setup_new_tle_by_spacetrack(station_name, norad_id)
    OrbiSat.setup_comm(station_name, norad_id)
    OrbiSat.predict_comm(station_name, norad_id)

    sessions_params = OrbiSat.get_comm_sessions_params(station_name, norad_id)
    comm_params = OrbiSat.get_all_data(station_name, norad_id)

    for i in range(10):
        time.sleep(1)
        data_angles = OrbiSat.get_azimuth_elevation(
            station_name, norad_id, datetime.utcnow()
        )
        data_freqs = OrbiSat.get_frequencies(station_name, norad_id, datetime.utcnow())
        data = OrbiSat.get_data(station_name, norad_id, datetime.utcnow())

    print("Finished!")
    OrbiSat.clear_ground_station_data(station_name)
    pass
